...
def discover_key(username_trial):
    global key_part_static1_trial
    global key_part_static2_trial
    global key_full_template_trial

    key_list = list(' ' * len(key_full_template_trial))
    i = 0
    ...
    for index in dynamic_indices:
        key_list[i] = hashed_username[index]
        i += 1

    # Assign static part 2
    for c in key_part_static2_trial:
        key_list[i] = c
        i += 1
...
# Example usage
username_trial = "SCHOFIELD"
key = discover_key(username_trial)
print(key)
